...
int fork_and_exec(char cmd[], int ar, char *argv[])
{
  int child_pid = fork();
  if (child_pid < 0)
  {
...
int check_run_command(wordexp_t we)
{
  ...
  if (strcmp(cmd_name, "cd") == 0)
    return cd(ar - 1, &argv[1]);

  if (strcmp(cmd_name, "echo") == 0)
    return echo(ar - 1, &argv[1]);

  if (strcmp(cmd_name, "history") == 0)
    return history(ar - 1, &argv[1]);

  if (strcmp(cmd_name, "pwd") == 0)
    return pwd(ar - 1, &argv[1]);

  if (strcmp(cmd_name, "exit") == 0)
    return exit_wrp(ar - 1, &argv[1]);

  if (strcmp(cmd_name, "ls") == 0)
    return fork_and_exec("/bin/ls", ar, argv);

  if (strcmp(cmd_name, "cat") == 0)
    return fork_and_exec("/bin/cat", ar, argv);

  if (strcmp(cmd_name, "mkdir") == 0)
    return fork_and_exec("/bin/mkdir", ar, argv);

  if (strcmp(cmd_name, "rm") == 0)
    return fork_and_exec("/bin/rm", ar, argv);

  if (strcmp(cmd_name, "date") == 0)
    return fork_and_exec("/bin/date", ar, argv);
